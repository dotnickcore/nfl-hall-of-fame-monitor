import { CHAMPIONSHIPS_WON_MULTIPLIER, DEFENSE_TEAMS_OTHER_SELECTION, DEFENSIVE_PLAYER_MULTIPLIER, EACH_YEAR_PLAYED_IN_AFL_PENALTY, FIRST_TEAM_DEFENSE_SELECTION, FIRST_TEAM_OFFENSE_SELECTION, FIRST_TEAM_SPECIAL_TEAMS_SELECTION, HALL_OF_FAME_BALLOT_FINALIST_BONUS, HALL_OF_FAME_BALLOT_SEMIFINALIST_BONUS, MOST_VALUABLE_PLAYER_MULTIPLIER, OFFENSE_TEAMS_OTHER_SELECTION, PRO_BOWL_MULTIPLIER, PRO_FOOTBALL_HALL_OF_FAME_ALL_DECADE_SELECTIONS_BONUS, SECOND_TEAM_DEFENSE_SELECTION, SECOND_TEAM_OFFENSE_SELECTION, SECOND_TEAM_SPECIAL_TEAMS_SELECTION, SPECIAL_TEAMS_OTHER_SELECTION, WEIGHTED_APPROXIMATE_VALUE_MULTIPLIER, ZERO } from "../../consts/common/common";
import { ICommon } from "../../interfaces/monitor/common/ICommon";
import { isPercentageOver } from "../../utils/isPercentageOver";
...
export abstract class CommonService implements ICommon {
    ...
    returnFirstTeamAssociatedPressAllProSelectionCalculation(careeringStartingSeasons: number, timesAwarded: number): number {
        if (isPercentageOver(timesAwarded, careeringStartingSeasons)) return PRO_FOOTBALL_HALL_OF_FAME_ALL_DECADE_SELECTIONS_BONUS;
        return ZERO;
    }
